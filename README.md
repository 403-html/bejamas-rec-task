...
- The test case "Verify that all links do not return 404 errors" specifies all pages, but I assumed that it should be done for the homepage only (the Netlify website has tons of pages, so it would be too much work to do it for all of them – missing the point of the recruitment task). I crawled all links on the homepage and created dynamic test suite to check them.
